...
################################################################################
# 순차 배포 스크립트 (Admin 서버) - 1GB RAM 최적화
# 
...
update_nginx_upstream() {
    local target_env=$1
    local target_container="admin-${target_env}"
    ...
    # upstream 설정 업데이트
    sed -i.tmp "s/server admin-[a-z]*:3000;/server ${target_container}:3000;/" "$NGINX_CONF"
    rm -f "${NGINX_CONF}.tmp"
...
validate_nginx_config() {
    ...
    if docker exec workly-admin-nginx nginx -t > /dev/null 2>&1; then
...
reload_nginx() {
    ...
    if docker exec workly-admin-nginx nginx -s reload; then
...
pull_image() {
    local env=$1
    local image_tag="admin-${env}"
    
    log_info "Docker 이미지 풀 중: ${image_tag}"
    
    cd "$DEPLOYMENT_DIR"
    if docker compose pull "admin-${env}"; then
...
stop_container() {
    ...
    cd "$DEPLOYMENT_DIR"
    if docker compose stop "admin-${env}"; then
...
start_container() {
    ...
    cd "$DEPLOYMENT_DIR"
    if docker compose up -d "admin-${env}"; then
