...
import type { SelectBoard, SelectBoardGroup } from "./useSelectBoardList";
...
export function normalizeAndSortBoards(list: Board[]): SelectBoard[] {
  return (list ?? [])
    .map<SelectBoard>((b) => ({
      id: b.boardId,
      name: b.boardName,
...
export function buildGroups(catMap: CatMap, boards: SelectBoard[]): SelectBoardGroup[] {
  const groups: SelectBoardGroup[] = Object.keys(catMap)
    .map((idStr) => {
      const id = Number(idStr);
      const meta = catMap[id];
      return { category: { id, name: meta.name, order: meta.order }, boards: [] as SelectBoard[] };
    })
